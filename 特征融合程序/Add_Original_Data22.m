function Add_Original_Data22() %添加数据
clc
close all
clear all
...
%% 原始数据
%故障1
x1=1*sin(x)+ 0.7*rand(1,len);
Fault1=reshape(x1,columns,rows);
%故障2
x2=3*sin(6*x)+ 0.9*rand(1,len);
Fault2=reshape(x2,columns,rows);
%故障3
x3=2.4*sin(8*x)+ 0.4*rand(1,len);
Fault3=reshape(x3,columns,rows);
%故障4
x4=2.8*sin(16*x)+ 0.9*rand(1,len);
Fault4=reshape(x4,columns,rows);
testData_original2=[Fault1,Fault2,Fault3,Fault4];
save testData_original2 testData_original2
%% 斜率数据
%故障1数据的斜率
x1_slope=zeros(1,len);
for i=1:len-1
...
Fault4_slope=reshape(x4_slope,columns,rows);
testData_slope2=[Fault1_slope,Fault2_slope,Fault3_slope,Fault4_slope];
save testData_slope2 testData_slope2
%% 曲率数据
%故障1数据的曲率
x1_curvature=zeros(1,len);
for i=1:len-1
...
Fault4_curvature=reshape(x4_curvature,columns,rows);
testData_curvature2=[Fault1_curvature,Fault2_curvature,Fault3_curvature,Fault4_curvature];
save testData_curvature2 testData_curvature2
%% 测试数据标签
