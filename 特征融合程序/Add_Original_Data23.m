function Add_Original_Data23() %添加数据
clc
close all
clear all
...
%% 原始数据
%故障1
x1=2*sin(x)+ 1*rand(1,len);
Fault1=reshape(x1,columns,rows);
%故障2
x2=2*sin(4*x)+ 1*rand(1,len);
Fault2=reshape(x2,columns,rows);
%故障3
x3=2.5*sin(8*x)+ 1*rand(1,len);
Fault3=reshape(x3,columns,rows);
%故障4
x4=2.8*sin(16*x)+ 0.8*rand(1,len);
Fault4=reshape(x4,columns,rows);
testData_original3=[Fault1,Fault2,Fault3,Fault4];
save testData_original3 testData_original3
%% 斜率数据
%故障1数据的斜率
x1_slope=zeros(1,len);
for i=1:len-1
...
Fault4_slope=reshape(x4_slope,columns,rows);
testData_slope3=[Fault1_slope,Fault2_slope,Fault3_slope,Fault4_slope];
save testData_slope3 testData_slope3
%% 曲率数据
%故障1数据的曲率
x1_curvature=zeros(1,len);
for i=1:len-1
...
Fault4_curvature=reshape(x4_curvature,columns,rows);
testData_curvature3=[Fault1_curvature,Fault2_curvature,Fault3_curvature,Fault4_curvature];
save testData_curvature3 testData_curvature3
%% 测试数据标签
