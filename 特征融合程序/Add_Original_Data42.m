function Add_Original_Data42() %添加数据
clc
close all
clear all
xzb=0.5;%高斯白噪声的信噪比
...
x2=awgn(y2,xzb); 
Fault2=reshape(x2,columns,rows);
testData_original42=[Fault1,Fault2];
save testData_original42 testData_original42
%% 斜率数据
%故障1数据的斜率
x1_slope=zeros(1,len);
for i=1:len-1
...
Fault2_slope=reshape(x2_slope,columns,rows);
testData_slope42=[Fault1_slope,Fault2_slope];
save testData_slope42 testData_slope42
%% 曲率数据
%故障1数据的曲率
x1_curvature=zeros(1,len);
for i=1:len-1
...
testData_curvature42=[Fault1_curvature,Fault2_curvature];
save testData_curvature42 testData_curvature42
%% 测试数据标签
