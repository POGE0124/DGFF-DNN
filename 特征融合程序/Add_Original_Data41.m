function Add_Original_Data41() %添加数据
clc
close all
clear all
xzb=1;%高斯白噪声的信噪比
...
x2=awgn(y2,xzb); 
Fault2=reshape(x2,columns,rows);
testData_original41=[Fault1,Fault2];
save testData_original41 testData_original41
%% 斜率数据
%故障1数据的斜率
x1_slope=zeros(1,len);
for i=1:len-1
...
Fault2_slope=reshape(x2_slope,columns,rows);
testData_slope41=[Fault1_slope,Fault2_slope];
save testData_slope41 testData_slope41
%% 曲率数据
%故障1数据的曲率
x1_curvature=zeros(1,len);
for i=1:len-1
...
testData_curvature41=[Fault1_curvature,Fault2_curvature];
save testData_curvature41 testData_curvature41
%% 测试数据标签
